import 'package:biori/main_screen/add/user_stories/event/events_group/dto/add_event_group_dto.dart';
import 'package:biori/main_screen/add/user_stories/event/events_group/repository/add_event_group_repository.dart';
...
class AddEventGroupPageVM {
  final eventGroupRepository = AddEventGroupRespository();
  ...
  AddEventGroupDto model = AddEventGroupDto(fechas: []);
  EventValidators eventValidators = EventValidators();
  JaviForms form = JaviForms();

  void loadTags() async {
    allGroups = await eventGroupRepository.getAllGroups();
  }
  ...
  onSavedValTags(List<ChipButtonModel> onSavedVal) {
    model.groupsId = onSavedVal.map((e) => e.id).toList();
  }
  ...
  void submitButton(BuildContext context) async {
    ...
    var addEventGroupOutput = await AddEventGroup().run(model);
    isApiCallProcess.add(false);

    if (context.mounted) {
      responseDialog
          .add(responseDialogFromOutput(context, addEventGroupOutput));
    }
  }

  ResponseDialog responseDialogFromOutput(
      BuildContext context, AddEventGroupOutput addEventOutput) {
    String titleDialog;
    Icon iconDialog;
    Function onPressed = () {
